...
# Adicione no início do arquivo, com as outras importações
import random
import numpy as np
import cupy as cp
...
def select_best_cluster(heuristic_matrix, clusters):
    """Seleciona o cluster com as melhores conexões médias"""
    cluster_qualities = []
    for c in np.unique(clusters):
        mask = (clusters == c)
        submatrix = heuristic_matrix[mask][:, mask]
        cluster_qualities.append(np.mean(submatrix))

    best_cluster = np.argmax(cluster_qualities)
    return best_cluster


def spectral_clustering(heuristic_matrix, n_clusters=3):
    """
    heuristic_matrix: matriz nxn onde valores mais altos indicam melhores conexões
    n_clusters: número de clusters para identificar
    """
    # 1. Normalizar a matriz heurística para similaridade [0,1]
    W = (heuristic_matrix - heuristic_matrix.min()) / (heuristic_matrix.max() - heuristic_matrix.min())
    ...
    # 4. Normalizar linhas e clusterizar
    U_norm = U / np.linalg.norm(U, axis=1)[:, np.newaxis]
    clusters = KMeans(n_clusters=n_clusters).fit_predict(U_norm)
...
def spectral_ant_colony(dados, iteracoes, formigas, evaporacao, feromonio, alfa, beta, n_clusters=3):
    print("Ant Colony with Spectral Clustering")

    # 1. Construir matriz heurística
    heuristica_mat = construir_heuristica(dados)
    heuristica_mat = 1-heuristica_mat
    print("Construiu heuristica")

    # 2. Aplicar clusterização espectral na matriz heurística
    clusters = spectral_clustering(heuristica_mat, n_clusters=n_clusters)
    best_cluster = select_best_cluster(heuristica_mat, clusters)
    print(f"Identificou clusters. Melhor cluster: {best_cluster}")

    # 3. Criar máscara para o melhor cluster
    cluster_nodes = np.where(clusters == best_cluster)[0]
    cluster_mask = np.zeros_like(heuristica_mat, dtype=bool)
    cluster_mask[np.ix_(cluster_nodes, cluster_nodes)] = True

    # 4. Inicializar matriz de feromônio com reforço no melhor cluster
    feromonio_mat = np.ones(heuristica_mat.shape)
    feromonio_mat[cluster_mask] *= 2.0  # Reforço inicial no cluster
    ...
    for iteracao in range(iteracoes):
        ...
        def tarefa(formiga_id):
            ...
            # Modificação: 80% de chance de começar no cluster
            if random.random() < 0.8 and cluster_nodes.size > 0:
                inicio = random.choice(cluster_nodes)
            else:
                inicio = random.choice(range(len(heuristica_mat)))

            solucao = construir_solucao_cluster(
                dados_local, formiga_id, heuristica_mat, feromonio_mat,
                alfa, beta, cluster_nodes, cluster_mask, inicio
            )
            valor = funcao_objetivo(dados_local)
            return solucao, valor, dados_local['wave']['pedidos'], dados_local['wave']['corredores'], \
            dados_local['wave']['tamanho']
        ...
        atualiza_feromonios_cluster(
            feromonio_mat, solucoes, valores, evaporacao, feromonio,
            cluster_mask, boost_factor=1.5
        )
...
def construir_solucao_cluster(dados, formiga, heuristica_mat, feromonio_mat,
                              alfa, beta, cluster_nodes, cluster_mask, inicio):
    ...
    while (not demanda_atendida(demanda, oferta) or
           pedidos_no_caminho == 0 or corredores_no_caminho == 0) and vertices_restantes:

        # Probabilidade de seguir no cluster
        if atual in cluster_nodes and random.random() < 0.7:
            candidatos = [v for v in vertices_restantes if v in cluster_nodes]
            if not candidatos:
...
def atualiza_feromonios_cluster(feromonio_mat, solucoes, valores, evaporacao,
                                feromonio, cluster_mask, boost_factor=1.5):
    feromonio_mat *= (1 - evaporacao)
    feromonio_mat[feromonio_mat < 0.1] = 0.1
    ...
    for solucao, valor in zip(solucoes, valores):
        if valor <= 0:
            continue
        score = (valor - val_min) / val_range
        delta = feromonio * score
        ...
        for i in range(len(solucao) - 1):
            if (0 <= solucao[i] < feromonio_mat.shape[0] and
                ...
                # Aumenta feromônio adicional para arestas no cluster
                if cluster_mask[solucao[i], solucao[i + 1]]:
                    delta_boosted = delta * boost_factor
                    cluster_count += 1
                else:
                    delta_boosted = delta

                feromonio_mat[solucao[i], solucao[i + 1]] += delta_boosted
                feromonio_mat[solucao[i + 1], solucao[i]] += delta_boosted

        # Reforço adicional para soluções com muitos elementos no cluster
        if cluster_count > len(solucao) * 0.7:
            for i in range(len(solucao) - 1):
                if cluster_mask[solucao[i], solucao[i + 1]]:
                    feromonio_mat[solucao[i], solucao[i + 1]] *= 1.2
                    feromonio_mat[solucao[i + 1], solucao[i]] *= 1.2